#!/usr/bin/env bash
# tui/disk_select.sh — Disk selection + partition scheme + shrink wizard
...
# _shrink_wizard — Interactive wizard for shrinking a partition to make room for NixOS
...
_shrink_wizard() {
    ...
    dialog_msgbox "No Free Space" \
        "There is not enough free space on ${disk} to create a NixOS partition.\n\n\
...
There is not enough room to free space for NixOS."
        return "${TUI_BACK}"
    fi

    local max_nixos_mib=$(( total_mib - min_part_size ))

    # Default: half available or 20 GiB, whichever is smaller
    local default_nixos=20480
    if [[ $(( max_nixos_mib / 2 )) -lt ${default_nixos} ]]; then
        default_nixos=$(( max_nixos_mib / 2 ))
    fi
    if [[ ${default_nixos} -lt ${NIXOS_MIN_SIZE_MIB} ]]; then
        default_nixos=${NIXOS_MIN_SIZE_MIB}
    fi

    # Step 6: Ask for NixOS size
    local nixos_size_mib
    while true; do
        nixos_size_mib=$(dialog_inputbox "Space for NixOS (MiB)" \
            "How much space to free for NixOS?\n\n\
Partition: ${selected_part} (${selected_fstype})\n\
Total:     ${total_mib} MiB\n\
Used:      ${used_mib} MiB\n\
Maximum:   ${max_nixos_mib} MiB\n\
Minimum:   ${NIXOS_MIN_SIZE_MIB} MiB (10 GiB)\n\n\
Enter size in MiB:" \
            "${default_nixos}") || return "${TUI_BACK}"

        if [[ ! "${nixos_size_mib}" =~ ^[0-9]+$ ]]; then
            dialog_msgbox "Invalid Input" "Please enter a number (MiB)." || true
            continue
        fi

        if [[ ${nixos_size_mib} -lt ${NIXOS_MIN_SIZE_MIB} ]]; then
            dialog_msgbox "Too Small" \
                "Minimum size for NixOS is ${NIXOS_MIN_SIZE_MIB} MiB (10 GiB)." || true
            continue
        fi

        if [[ ${nixos_size_mib} -gt ${max_nixos_mib} ]]; then
            dialog_msgbox "Too Large" \
                "Maximum available space is ${max_nixos_mib} MiB.\n\
(Total ${total_mib} - Used ${used_mib} - 1 GiB safety margin)" || true
    ...
    local new_part_size=$(( total_mib - nixos_size_mib ))
  ...
  Freed space:  ${nixos_size_mib} MiB (for NixOS)\n\n\
Proceed with shrink?" \
        || return "${TUI_BACK}"
...
Type ERASE in the next dialog to confirm."
                    ...
                    if [[ ${free_mib} -lt ${NIXOS_MIN_SIZE_MIB} ]]; then
