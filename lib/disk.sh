    ...
    case "${fstype}" in
        ...
        btrfs)
            disk_plan_add "Shrink btrfs filesystem on ${part}" \
                bash -c "mkdir -p /mnt/nixos-shrink-tmp && mount ${part} /mnt/nixos-shrink-tmp && btrfs filesystem resize ${new_size}M /mnt/nixos-shrink-tmp && umount /mnt/nixos-shrink-tmp"
            ;;
    esac

    # Resize partition table entry via sfdisk
