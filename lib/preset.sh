...
readonly -a PRESET_HW_VARS=(
    CPU_VENDOR CPU_MODEL CPU_CORES
    GPU_VENDOR GPU_DEVICE_ID GPU_DEVICE_NAME GPU_DRIVER GPU_NVIDIA_OPEN
    HYBRID_GPU IGPU_VENDOR IGPU_DEVICE_NAME IGPU_BUS_ID
    DGPU_VENDOR DGPU_DEVICE_NAME DGPU_BUS_ID
    ASUS_ROG_DETECTED
    TARGET_DISK ESP_PARTITION ROOT_PARTITION SWAP_PARTITION ESP_REUSE LUKS_PARTITION
)
