...
# correctly (boot menu, console, SDDM/Plasma/GNOME). Also flags ALC287
...
detect_umpc() {
    ...
    if [[ "${UMPC_DETECTED}" == "1" ]]; then
        ...
        [[ "${UMPC_ALC287_QUIRK}" == "1" ]] && einfo "  ALC287 Auto-Mute quirk: will declare runtime fix service"
        [[ "${UMPC_GPD_FAN}" == "1" ]] && einfo "  GPD fan: will add config comment with manual setup instructions"
    fi
...
get_hardware_summary() {
    ...
    if [[ "${UMPC_DETECTED:-0}" == "1" ]]; then
        s+="  UMPC: ${UMPC_VENDOR} ${UMPC_MODEL}\n"
        if [[ -n "${UMPC_PANEL_ORIENTATION:-}" ]]; then
            s+="    Panel rotation fix: ${UMPC_PANEL_ORIENTATION} on ${UMPC_VIDEO_CONNECTOR}\n"
        fi
    fi
