...
elog()    { _log "LOG"   "${CYAN}"   "$@"; }
einfo()  { _log "INFO"  "${GREEN}"  "$@"; }
ewarn()  { _log "WARN"  "${YELLOW}" "$@"; }
eerror() { _log "ERROR" "${RED}"    "$@"; }
...
log_relocate_to_target() {
    ...
    local target="${target_dir}/nixos-installer.log"
    ...
    LOG_FILE="${target}"
    SKIPPED_LOG="${target_dir}/nixos-installer-skipped.log"
...
skipped_step_record() {
    local desc="$1"
    : "${SKIPPED_LOG:=/tmp/nixos-installer-skipped.log}"
...
skipped_steps_report() {
    local f="${SKIPPED_LOG:-/tmp/nixos-installer-skipped.log}"
    [[ -s "${f}" ]] || return 0
